...
class AggregateAssignmentMatrixGenerator:
    ...
    def count_matrices(self, n_src_conn, n_tgt_conn) -> int:
        ...
        i_src_end = len(n_src_conn)-1

        def _count_for_src(i_src, n_tgt_remain):
            # Determine how many times we can connect to each target slot
            n_src_to_target = n_tgt_remain.copy()
            ...
            if i_src == i_src_end:
                return n_taken_combs.shape[0]

            # Branch out
            count = 0
            for i in range(n_taken_combs.shape[0]):
                count += _count_for_src(i_src+1, n_tgt_remain-n_taken_combs[i, :])

            return count

        return _count_for_src(0, np.array(n_tgt_conn))
...
def count_n_pool_take(n_pool, n_take, n_dup_next: Tuple[int, ...]):
    """
    Number of combinations taking n_take elements from a pool of size n_pool, where some of the slots might be referring
    to the same physical slot (i.e. are duplicate) and therefore only the nr in that slot matters, not the order inside.

    For explaining we use the following notation: n_pool = 3, n_take = 2 --> take 2 from 3
    Different combinations are written as binary strings indicating which slot has been taken.

    Example: take 2 from 3 --> 110; 101; 011 --> 3 combinations

    The n_dup_next array indicates for each slot how many of the following slots are referring to the same physical
    slots, and therefore should be regarded as duplicated from the original slot. Example:
    take 2 from 3, 100dup (this means that the first slot has one duplicated slot after it)
    110
    101
    011 --> this is a duplicate from the first combination, so we don't count

    Another example: take 3 from 6, 100200dup
    111000
    110100
    101100
    100110
    ...
    n_comb, combinations = _count_n_pool_take(n_pool, n_take, n_dup_next)
    ...
    if is_swapped:
        ...
        combinations = np.bitwise_not(combinations)[::-1, :]

    return n_comb, combinations


@functools.lru_cache(maxsize=None)
def _count_n_pool_take(n_pool, n_take, n_dup_next: Tuple[int, ...]):
    ...
    # Special case: if we are taking only one, there are n_pool combinations, not counting any duplicates
    # Example: take 1 from 4        --> 1000; 0100; 0010; 0001
    #          take 1 from 4, 0100d --> 1000; 0100;       0001
    if n_take == 1:
        combinations = np.eye(n_pool, dtype=bool)

        # Take out combinations in a duplicated slot
        mask_keep = np.ones((n_pool,), dtype=bool)
        for i, n_dup in enumerate(n_dup_next):
            if n_dup > 0:
                mask_keep[i+1:i+1+n_dup] = False
        combinations = combinations[mask_keep, :]

        return n_pool-np.sum(n_dup_next), combinations

    # Deal with our current slot having duplicates
    if n_dup_next[0] > 0:
        # Count the number of slots that are duplicated (including this slot)
        n_pool_dup = n_dup_next[0]+1

        # Count the number of slots that are located after this set of duplicated slots
        n_pool_after = n_pool-n_pool_dup

        # If there are no elements after the duplicated slots, we only have one combination (which is taking all from
        # the duplicated slots)
        if n_pool_after <= 0:
            comb_take = np.zeros((n_pool,), dtype=bool)
            comb_take[:n_take] = 1
            return 1, np.array([comb_take])

        # Calculate the minimum number of taken elements in the duplicated slots, checking that there cannot be too many
        # taken from the slots after the duplicated slots
        n_min_in_dup = max(0, n_take-n_pool_after)

        # Calculate the maximum number of taken elements in the duplicated slots, checking that it is not more than the
        # number of elements we want to take
        n_max_in_dup = min(n_take, n_pool_dup)

        # Count number of combinations for different number of elements in the duplicated slots
        n_comb = 0
        combinations = []
        n_dup_after = n_dup_next[n_pool_dup:]
        for n_in_dup in reversed(range(n_min_in_dup, n_max_in_dup+1)):
            # Calculate the number of elements taken from the slots after the duplicated slots
            n_take_after = n_take-n_in_dup

            # Count number of combinations from slots after the duplicate slots
            next_n_comb, next_combs = _count_n_pool_take(n_pool_after, n_take_after, n_dup_after)
            n_comb += next_n_comb

            # Combinations matrix is constructed from part in the duplication (zeros with n_in_dup leading ones)
            comb_dup_prefix = np.zeros((next_combs.shape[0], n_pool_dup), dtype=bool)
            comb_dup_prefix[:, :n_in_dup] = 1
            combinations.append(np.column_stack([comb_dup_prefix, next_combs]))

        return n_comb, np.row_stack(combinations)

    # Count number of combinations by recursively adding combinations from taking one less element from different pool
    # sizes; Example: take 3 from 5
    # 11100 \               \
    # 11010 |               | take 1 from 3
    # 11001 | take 2 from 4 /
    # 10110 |               \               \ take 1 from 2
    ...
    # 10011 /               /               - take 2 from 2
    # 01110 \               \               \ take 1 from 2
    # 01101 | take 3 from 4 | take 2 from 3 /
    # 01011 |               /               - take 2 from 2
    # 00111 /               - take 3 from 3
    n_comb = 0
    combinations = []

    one_n_comb, next_combs = _count_n_pool_take(n_pool-1, n_take-1, n_dup_next[1:])
    n_comb += one_n_comb
    one_comb_prefix = np.ones((next_combs.shape[0], 1), dtype=bool)
    combinations.append(np.column_stack([one_comb_prefix, next_combs]))

    zero_n_comb, next_combs = _count_n_pool_take(n_pool-1, n_take, n_dup_next[1:])
    n_comb += zero_n_comb
    zero_comb_prefix = np.zeros((next_combs.shape[0], 1), dtype=bool)
    combinations.append(np.column_stack([zero_comb_prefix, next_combs]))
